...
namespace Implementation7
{
    public class WhiteDwarf : Star
    {
        ...
        public WhiteDwarf(string objectName, double mass, int temprature)
        {
        ...
        {
            return $"В Наблюдаемой Вселенной насчитывают около 0,6068 процентов звезд, принадлежащих к спектральному классу \"{SpectralСlass.ClassName}\"!";
        }

        public string GetVisibleColor()
        {
            return $"Истинный цвет звезд данного типа: \"{StarColorExtensions.ToString(SpectralСlass.Color)}\", в то время как видим мы: \"Белый\"!";
        }

        public static BlueGiant CreateRandomWhiteDwarf(int nameSize)
        {
            Random random = new Random();
            return new BlueGiant(GetRandomName(nameSize, random), random.NextDouble() * 1.4 + 1.7, random.Next(7500, 10000));
        }
